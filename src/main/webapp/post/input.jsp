...
<body>
  <div class="wrapper">
    ...
    <main>
      ...
      <form action="<%=request.getContextPath()%>/post.do?m=insert" method="post" enctype="multipart/form-data">   
        <div style="border-top:2px solid black;border-bottom:2px solid black;padding-top:20px;padding-bottom:20px;">
          <label style="font-size:46px; margin:10px; margin-bottom:0px">
            <c:choose>
              <c:when test="${category_num eq '1'}">자유게시판</c:when>
              <c:when test="${category_num eq '2'}">입시정보 게시판</c:when>
              <c:when test="${category_num eq '3'}">고등학교 게시판</c:when>
              <c:when test="${category_num eq '4'}">공지사항</c:when>
