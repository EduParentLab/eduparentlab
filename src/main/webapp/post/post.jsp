  ...
  <div class="wrapper">
    ...
    <div class="section-discription">
      <c:choose>
		  ...
		  <div class="board-discription-box">
		    <label>💬 이곳은 학부모 정보통의 자유게시판입니다.</label>
		    <label>아이들의 배움과 성장을 위해, 부모가 함께 지혜를 나누는 곳.</label>
		    <label>입시 정보부터 생활 학습 팁까지 자유롭게 공유하며, 서로의 경험이 모두의 자산이 됩니다. 🌟</label>
		  </div>
		</c:when>
        <c:when test="${category_num eq '2'}">
		  <div class="board-discription-box">
		    <label>🎓 이곳은 입시에 관심 있는 모든 학부모와 학생들을 위한 열린 공간입니다.</label>
		    <label>최신 입시 정보와 소식을 확인할 수 있을 뿐만 아니라, 서로의 경험과 고민을 나누며 함께 성장하는 커뮤니티입니다.</label>
		    <label>입시는 혼자 준비하면 막막할 수 있지만, 🤗 함께 나누는 정보와 응원은 분명히 큰 힘이 됩니다.</label>
		    <label>궁금한 점을 자유롭게 질문하시고, 알고 있는 정보를 나누어 주시면, 모두가 더 나은 길을 찾아갈 수 있습니다. ✨</label>
		  </div>
		</c:when>      
        <c:when test="${category_num eq '3'}">
		  <div class="board-discription-box">
		    <label>🎓 이곳은 고등학교 생활과 관련된 다양한 이야기를 나누는 공간입니다.</label>
		    <label>📚 공부 방법, 🏫 학교 생활 팁, 🎭 동아리 활동, 📝 시험 후기 등 학생과 학부모님들이 함께 공감하고 공유할 수 있는 주제를 자유롭게 올려주세요.</label>
		    <label>🤝 혼자 고민하기보다 함께 나누고 듣는 이야기가 더 큰 힘이 됩니다.</label>
		  </div>
		</c:when>  
        <c:when test="${category_num eq '4'}">
		  <div class="board-discription-box">
		    <label>📢 이곳은 학부모 정보통의 <b>공식 공지사항 게시판</b>입니다.</label>
		    <label>사이트 운영 일정, 새로운 기능 추가, 서비스 점검 등 <b>중요한 안내사항</b>을 신속하고 정확하게 전달합니다.</label>
		...
		<c:if test="${loginOkUser.email=='admin@edu_parent.com' or category_num != '4'}">
	        <div class="section-write-btn" style="gap:10px">
	          ...
	          <button class="write-btn" 
        		 onclick="location.href='<%=request.getContextPath()%>/post.do?m=input&category_num=${param.category_num}'">글쓰기
        	  </button>
	        </div>
		</c:if>
